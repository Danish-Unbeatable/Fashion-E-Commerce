...
if (isset($_POST['submit']))
    {
        if($_SESSION['name'])
        {
                ...
                $product_id=$_POST['product_id'];// name field from form
                ...
                $result=mysqli_query($conn,$sql2);
                    if(mysqli_num_rows($result) > 0)
                        { 
                            ...
                            {
                                $user_name=$row['name']; // col from user table
                                $mobile=$row['mobile'];
                                $address=$row['address'];
                                $email=$row['email'];
...
<head>
    <meta charset="UTF-8">
    <title>Fashion Lifestyle-buy product</title>
    <meta name="view
...
<body>
        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-md-7">
                    ...
                    <h2><?php echo $row['product']?><!--Women's crop top- white color--></h2>
                    ...
                    <p>Bank Offer5% Unlimited Cashback on Flipkart Axis Bank Credit CardT&C</p>
                      ...
                      <p> No Cost EMI on Flipkart Axis Bank Credit CardT&C</p>-->
                    <p><b>Brand : </b><?php echo $row['brand']?> </p>
                    <p><b>Size: </b></p>
                    <div class="btn-group" data-toggle="buttons">
                    <label class=" btn btn-primary">
                    <input type="radio" name="size" class="form-control"
                        value=""><?php echo $row['size'][0]?>
                        </label>
                        <input type="button" name="size" 
                        value="<?php echo $row['size'][1]?>">M
                        <input type="button" name="size" 
                        value="<?php echo $row['size'][2]?>">L
                        <input type="button" name="size" 
                        value="<?php echo $row['size'][3]?>L">XL 
</div>
    ...
    <section class="product-des">
            <div class="container">
                ...
                <p><?php echo $row['des']?><!--Welcome to Brackets, a modern open-source code editor that understands web design. It's a lightweight,
            ...
            <div class="row">
                <?php
                    $sql="SELECT * FROM trends order by rand() limit 4";
                    $result=mysqli_query($conn,$sql);
                    if (mysqli_num_rows($result)>0)
                    {
                        foreach($result as $row)
